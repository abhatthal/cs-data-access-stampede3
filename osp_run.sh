...
apptainer exec $HOST_BINDS "$IMAGE" bash -c "set -o pipefail; cd $CONTAINER_HOME && python3 cs-data-tools/src/input_gen/run_input_gen.py $INPUT_GEN_ARGS | python3 cs-data-tools/src/retrieve_cs_data.py -i - -o $CONTAINER_HOME/outputs -t $CONTAINER_HOME/tmp -c cs-data-tools/src/db_wrapper/tacc.cfg"
